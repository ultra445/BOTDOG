...
from .strategies import EXECUTION_PHASE_POST, EXECUTION_PHASE_PRE, ExecMode, LimitStyle, RunnerCtx, Slot
...
REPORT_COLUMNS = ["timestamp", "status", "sheet", "row", "strategy_id", "field", "message", "severity"]
...
def create_strategy_excel_template(path: Path | str = DEFAULT_STRATEGY_EXCEL_PATH) -> Path:
    target = Path(path)
    ...
    rows = {
        ...
        "GlobalSettings": [
            ...
            ["PRE_LADDER_STEPS", "47,30,21,14", "Default PRE ladder steps"],
        ...
        "README": [
            ...
            ["LIMIT uses price_mode LIMIT_LTP or LIMIT_THEO."],
...
def export_strategy_slots_to_excel(
    ...
    *,
    ...
    _write_xlsx(target, workbook_rows)
...
def _slot_order_fields(slot: Slot) -> tuple[str, str, float | None, str, float | None]:
...
def load_excel_strategy_slots(
    ...
    *,
    ...
    if not workbook_path.exists():
        ...
        result = StrategyExcelLoadResult([], [issue], 0, 0, 0, 0, {})
        if write_report:
    ...
    if issues:
        result = StrategyExcelLoadResult([], issues, 0, 0, 0, 0, {})
        if write_report:
            write_strategy_excel_report(result, report_path)
        raise StrategyExcelConfigError(result.errors)
    ...
    for index, row in enumerate(strategy_rows, start=2):
        ...
        if order_mode == "LIMIT" and price_mode not in {"LIMIT_LTP", "LIMIT_THEO"}:
            issues.append(_issue("Strategies", index, sid, "price_mode", "LIMIT requires LIMIT_LTP or LIMIT_THEO"))
...
def write_strategy_excel_report(
    ...
    rows = result.issues or [
        StrategyExcelIssue("OK", "Workbook", "", "", "load", f"active_strategies={result.active_count}", "INFO")
    ]
    with report_path.open("w", newline="", encoding="utf-8") as handle:
...
def _slot_from_excel_row(
    ...
    slot.price_limit_factor = sp_limit_multiplier

...
def _trap_is(ctx: RunnerCtx, k: int) -> bool:
    try:
        return int(getattr(ctx, "trap", 0)) == int(k)
    except Exception:
        return False

# ================= TRAP1 systems (ROW) =================
def cond_trap1_laywin_1(ctx: RunnerCtx) -> bool:
    if ctx.market_type.upper() != "WIN" or ctx.milestone != 2: return False
    if getattr(ctx, "region", None) != "ROW" or int(getattr(ctx, "trap", 0)) != 1: return False
    wp = getattr(ctx, "winbet", None)
    if wp is None or not (7.0 <= wp < 12.0): return False
    evp = getattr(ctx, "ev_place", None)
    return (evp is not None) and (evp > 0.00)

def cond_trap1_laywin_2(ctx: RunnerCtx) -> bool:
    if ctx.market_type.upper() != "WIN" or ctx.milestone != 2: return False
    if getattr(ctx, "region", None) != "ROW" or int(getattr(ctx, "trap", 0)) != 1: return False
    wp = getattr(ctx, "winbet", None)
    if wp is None or not (12.0 <= wp < 20.0): return False
    evp = getattr(ctx, "ev_place", None)
    return (evp is not None) and (evp > -0.10)

def cond_trap1_laywin_3(ctx: RunnerCtx) -> bool:
    if ctx.market_type.upper() != "WIN" or ctx.milestone != 2: return False
    if getattr(ctx, "region", None) != "ROW" or int(getattr(ctx, "trap", 0)) != 1: return False
    wp = getattr(ctx, "winbet", None)
    if wp is None or not (20.0 <= wp < 50.0): return False
    evp = getattr(ctx, "ev_place", None)
    return (evp is not None) and (evp > -0.20)

def register_trap1_laywin(registry: List[Slot]):
    registry.append(Slot(family="LAY_WIN", slot=441, side=Side.LAY,
        condition=cond_trap1_laywin_1, exec_mode=ExecMode.HYB, limit_style=LimitStyle.AGGRESSIVE,
        price_for_bounds="WINBET", bet_per_market=False, edge_env="EDGE_TRAP1LAYWIN1_ROW",
        max_runner_stake_env="MAX_RUNNER_STAKE_TRAP1LAYWIN1_ROW"))
    registry.append(Slot(family="LAY_WIN", slot=442, side=Side.LAY,
        condition=cond_trap1_laywin_2, exec_mode=ExecMode.HYB, limit_style=LimitStyle.AGGRESSIVE,
        price_for_bounds="WINBET", bet_per_market=False, edge_env="EDGE_TRAP1LAYWIN2_ROW",
        max_runner_stake_env="MAX_RUNNER_STAKE_TRAP1LAYWIN2_ROW"))
    registry.append(Slot(family="LAY_WIN", slot=443, side=Side.LAY,
        condition=cond_trap1_laywin_3, exec_mode=ExecMode.HYB, limit_style=LimitStyle.AGGRESSIVE,
        price_for_bounds="WINBET", bet_per_market=False, edge_env="EDGE_TRAP1LAYWIN3_ROW",
        max_runner_stake_env="MAX_RUNNER_STAKE_TRAP1LAYWIN3_ROW"))

def cond_trap1_placelay_1(ctx: RunnerCtx) -> bool:
    if ctx.market_type.upper() != "PLACE" or ctx.milestone != 2: return False
    if getattr(ctx, "region", None) != "ROW" or int(getattr(ctx, "trap", 0)) != 1: return False
    p = _pick_bounds_price(ctx, "PLACE_BSP_THEN_LTP")
    return (p is not None) and (4.8 <= p < 7.0)

def cond_trap1_placelay_2(ctx: RunnerCtx) -> bool:
    if ctx.market_type.upper() != "PLACE" or ctx.milestone != 2: return False
    if getattr(ctx, "region", None) != "ROW" or int(getattr(ctx, "trap", 0)) != 1: return False
    p = _pick_bounds_price(ctx, "PLACE_BSP_THEN_LTP")
    return (p is not None) and (7.0 <= p < 15.0)

def cond_trap1_placelay_3(ctx: RunnerCtx) -> bool:
    if ctx.market_type.upper() != "PLACE" or ctx.milestone != 2: return False
    if getattr(ctx, "region", None) != "ROW" or int(getattr(ctx, "trap", 0)) != 1: return False
    p = _pick_bounds_price(ctx, "PLACE_BSP_THEN_LTP")
    return (p is not None) and (p >= 15.0)

def register_trap1_placelay(registry: List[Slot]):
    registry.append(Slot(family="LAY_PLACE", slot=451, side=Side.LAY,
        condition=cond_trap1_placelay_1, exec_mode=ExecMode.LIM, price_for_bounds="PLACE_BSP_THEN_LTP",
        bet_per_market=False, edge_env="EDGE_TRAP1LAYPLACE1_ROW",
        sp_limit_fn=lambda ctx: _lim_lay_from_place_theo(ctx, 0.70)))
    registry.append(Slot(family="LAY_PLACE", slot=452, side=Side.LAY,
        condition=cond_trap1_placelay_2, exec_mode=ExecMode.LIM, price_for_bounds="PLACE_BSP_THEN_LTP",
        bet_per_market=False, edge_env="EDGE_TRAP1LAYPLACE2_ROW",
        sp_limit_fn=lambda ctx: _lim_lay_from_place_theo(ctx, 0.75)))
    registry.append(Slot(family="LAY_PLACE", slot=453, side=Side.LAY,
        condition=cond_trap1_placelay_3, exec_mode=ExecMode.LIM, price_for_bounds="PLACE_BSP_THEN_LTP",
        bet_per_market=False, edge_env="EDGE_TRAP1LAYPLACE3_ROW",
        sp_limit_fn=lambda ctx: _lim_lay_from_place_theo(ctx, 0.85)))

def cond_trap1_backplace_1(ctx: RunnerCtx) -> bool:
    if ctx.market_type.upper() != "PLACE" or ctx.milestone != 2: return False
    if getattr(ctx, "region", None) != "ROW" or int(getattr(ctx, "trap", 0)) != 1: return False
    p = _pick_bounds_price(ctx, "PLACE_BSP_THEN_LTP")
    return (p is not None) and (2.0 <= p < 3.0)

def cond_trap1_backplace_2(ctx: RunnerCtx) -> bool:
    if ctx.market_type.upper() != "PLACE" or ctx.milestone != 2: return False
    if getattr(ctx, "region", None) != "ROW" or int(getattr(ctx, "trap", 0)) != 1: return False
    p = _pick_bounds_price(ctx, "PLACE_BSP_THEN_LTP")
    return (p is not None) and (3.0 <= p < 4.8)

def register_trap1_backplace(registry: List[Slot]):
    registry.append(Slot(family="BACK_PLACE", slot=461, side=Side.BACK,
        condition=cond_trap1_backplace_1, exec_mode=ExecMode.LIM, price_for_bounds="PLACE_BSP_THEN_LTP",
        bet_per_market=False, edge_env="EDGE_TRAP1BACKPLACE1_ROW",
        sp_limit_fn=lambda ctx: _lim_from_place_theo(ctx, 1.20)))
    registry.append(Slot(family="BACK_PLACE", slot=462, side=Side.BACK,
        condition=cond_trap1_backplace_2, exec_mode=ExecMode.LIM, price_for_bounds="PLACE_BSP_THEN_LTP",
        bet_per_market=False, edge_env="EDGE_TRAP1BACKPLACE2_ROW",
        sp_limit_fn=lambda ctx: _lim_from_place_theo(ctx, 1.15)))
